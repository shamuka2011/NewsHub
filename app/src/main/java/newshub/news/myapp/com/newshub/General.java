...
public class General extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        final RecyclerView recyclerView = view.findViewById(R.id.generalrecyclerview);
        recyclerView.setLayoutManager(new LinearLayoutManager(getActivity()));
        recyclerView.addItemDecoration(new DividerItemDecoration(getActivity(), LinearLayoutManager.VERTICAL));
        Call<GeneralResponse> call = apireq.getGeneralNews(API_KEY);

        call.enqueue(new Callback<GeneralResponse>() {
            @Override
            public void onResponse(Call<GeneralResponse> call, Response<GeneralResponse> response) {
                if (response.body().getStatus().equals("ok")) {
                    List<GeneralModel> responselist = response.body().getSources();
                    if (responselist.size()>0){
                        GeneralAdapter adapter = new GeneralAdapter(responselist);
